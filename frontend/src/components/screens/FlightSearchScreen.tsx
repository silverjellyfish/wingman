import svgPaths from "../imports/svg-xblwqlt639";
...
function Frame51({ onSelect }: { onSelect: () => void }) {
  return (
    <button 
      onClick={onSelect}
      className="bg-zinc-800 box-border content-stretch flex flex-col gap-[16px] items-start p-[12px] relative rounded-[16px] shrink-0 w-[336px] hover:bg-zinc-700 transition-colors"
    >
      <Frame63 />
    </button>
  );
...
function Frame52({ onNavigate }: { onNavigate: (screen: string) => void }) {
  ...
  return (
    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-[336px]">
      {[...Array(3).keys()].map((_, i) => (
        <Frame51 key={i} onSelect={handleFlightSelect} />
      ))}
    </div>
  );
}
...
export default function FlightSearchScreen({ onNavigate }: FlightSearchScreenProps) {
  return (
    <div className="bg-[#16161b] relative rounded-[40px] size-full" data-name="Trip 06">
      <div className="flex flex-col items-center relative size-full">
