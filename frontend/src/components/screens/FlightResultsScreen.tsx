...
import svgPaths from "../imports/svg-0g7z7q7l8g";
      ...
      <div className="flex flex-col justify-center overflow-clip relative size-full">
        <div className="box-border content-stretch flex flex-col items-start justify-center pl-0 pr-[258.05px] py-0 relative size-full">
          <div className="flex flex-col font-['Geist:Medium',_sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[14px] text-neutral-50 text-nowrap tracking-[0.07px]">
            <p className="leading-none whitespace-pre">07/20/2024</p>
          </div>
        </div>
      </div>
    </div>
...
function Button({ onClear }: { onClear: () => void }) {
  return (
    <button 
      onClick={onClear}
      className="box-border content-stretch flex h-[40px] items-center justify-center pb-[10.5px] pt-[9.5px] px-[16px] relative rounded-[6px] shrink-0 hover:bg-zinc-800 transition-colors cursor-pointer" 
      data-name="Button"
    >
      <div className="flex flex-col font-['Inter:Medium',_sans-serif] font-medium justify-center leading-[0] not-italic relative shrink-0 text-[14px] text-center text-neutral-50 text-nowrap">
        <p className="leading-[20px] whitespace-pre">Clear</p>
      </div>
    </button>
  );
}

function Frame48({ onClear }: { onClear: () => void }) {
  return (
...
function Frame6() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Frame">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Frame">
          <path d={svgPaths.p3e6cf80} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
...
function Button1({ onSelect }: { onSelect: () => void }) {
  return (
    <button 
      onClick={onSelect}
      className="bg-neutral-50 box-border content-stretch flex gap-[8px] h-[40px] items-center justify-center pb-[10.5px] pt-[9.5px] px-[12px] relative rounded-[6px] shrink-0 hover:bg-zinc-200 active:bg-zinc-300 transition-colors cursor-pointer" 
      data-name="Button"
    >
      <Frame6 />
      <div className="flex flex-col font-['Geist:Medium',_sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[14px] text-center text-nowrap text-zinc-900 tracking-[0.07px]">
        <p className="leading-none whitespace-pre">To the Airport</p>
      </div>
    </button>
  );
}
...
function Frame10() {
  return (
    <div className="relative shrink-0 size-[20px]" data-name="Frame">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 20 20">
        <g id="Frame">
          <path d={svgPaths.p3f82a500} fill="var(--fill-0, black)" id="Vector" />
        </g>
      </svg>
    </div>
  );
...
function Button2({ onSelect, isDisabled = false }: { onSelect: () => void; isDisabled?: boolean }) {
  return (
    <button 
      onClick={isDisabled ? undefined : onSelect}
      disabled={isDisabled}
      className={`box-border content-stretch flex gap-[8px] h-[40px] items-center justify-center pb-[10.5px] pt-[9.5px] px-[12px] relative rounded-[6px] shrink-0 transition-colors ${
        isDisabled 
          ? 'bg-zinc-700 cursor-not-allowed opacity-50' 
          : 'bg-neutral-50 hover:bg-zinc-100'
      }`}
      data-name="Button"
    >
      <Frame10 />
      <div className={`flex flex-col font-['Geist:Medium',_sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[14px] text-center text-nowrap tracking-[0.07px] ${
        isDisabled ? 'text-zinc-400' : 'text-zinc-900'
      }`}>
        <p className="leading-none whitespace-pre">From the Airport</p>
      </div>
    </button>
  );
}

function Frame50({ onSelect }: { onSelect: () => void }) {
  return (
    <div className="content-stretch flex items-center justify-between relative shrink-0 w-full">
      <Button1 onSelect={onSelect} />
      <Button2 onSelect={() => {}} isDisabled={true} />
    </div>
  );
}

function Frame51({ index, selectedFlight, onFlightClick, onButtonClick }: { 
  index: number; 
  ...
  return (
    <div className="bg-zinc-800 box-border content-stretch flex flex-col gap-[16px] items-start p-[12px] relative rounded-[16px] shrink-0 w-[336px] hover:bg-zinc-700 transition-colors cursor-pointer" onClick={() => onFlightClick(index)}>
      <Frame63 />
      {isSelected && <Frame50 onSelect={onButtonClick} />}
    </div>
  );
}

function Frame53({ onNavigate }: { onNavigate: (screen: string) => void }) {
  ...
  return (
    <div className="content-stretch flex flex-col gap-[12px] items-start relative shrink-0 w-[336px]">
      ...
      <div className="flex flex-col items-center relative size-full">
        <div className="box-border content-stretch flex flex-col gap-[40px] items-center px-[12px] py-[40px] relative size-full">
          <div className="flex flex-col font-['Geist:SemiBold',_sans-serif] font-semibold justify-center leading-[0] relative shrink-0 text-[24px] text-center text-nowrap text-white tracking-[0.12px]">
            <p className="leading-none whitespace-pre">Select Flight</p>
          </div>
          <Frame48 onClear={handleClear} />
          <Frame53 onNavigate={onNavigate} />
        </div>
      </div>
    </div>
  );
...
export default function FlightResultsScreen({ onNavigate }: FlightResultsScreenProps) {
  return (
    <div className="bg-[#16161b] relative rounded-[40px] size-full" data-name="Trip 05">
      <div className="flex flex-col items-center relative size-full">
