...
import svgPaths from "../imports/svg-bth51g4f77";
...
function Toast({ message, isVisible, onClose }: { message: string; isVisible: boolean; onClose: () => void }) {
  ...
  return (
    <div className={`fixed top-4 left-1/2 transform -translate-x-1/2 z-50 transition-all duration-300 ease-in-out ${
      ...
      <div className="bg-green-600 text-white px-6 py-3 rounded-lg shadow-lg font-['Geist:Medium',_sans-serif] font-medium text-[14px]">
        {message}
      </div>
    </div>
  );
...
function Input({ onSearch }: { onSearch: () => void }) {
  return (
    <button 
      onClick={onSearch}
      className="bg-zinc-950 h-[40px] relative rounded-[6px] shrink-0 w-full hover:bg-zinc-900 transition-colors cursor-pointer" 
      data-name="Input"
    >
...
function Frame41() {
  return (
    <div className="content-stretch flex items-center justify-between leading-[0] relative shrink-0 text-[14px] text-center text-neutral-50 text-nowrap tracking-[0.07px] w-full">
...
function Frame61() {
  return (
    <div className="[grid-area:1_/_1] bg-zinc-800 box-border content-stretch flex flex-col gap-[16px] items-start ml-0 mt-0 p-[12px] relative rounded-[16px] w-[345px]">
      <Frame63 />
    </div>
  );
}
...
function Group2() {
  return (
    <div className="grid-cols-[max-content] grid-rows-[max-content] inline-grid leading-[0] place-items-start relative shrink-0 w-full z-[2]">
      <Frame61 />
      <div className="[grid-area:1_/_1] h-0 ml-0 mt-[94px] relative w-[345px]">
        <div className="absolute inset-[-4px_-1.16%]" style={{ "--stroke-0": "rgba(22, 22, 27, 1)" } as React.CSSProperties}>
          <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 353 8">
            <path d={svgPaths.p13db9880} fill="var(--stroke-0, #16161B)" id="Line 3" />
          </svg>
        </div>
      </div>
    </div>
...
function Frame12() {
  return (
    <div className="content-stretch flex items-center leading-[0] relative shrink-0 text-[14px] text-center text-neutral-50 text-nowrap tracking-[0.07px] w-full">
      ...
      <div className="font-['Geist:Medium',_sans-serif] font-medium overflow-ellipsis overflow-hidden relative shrink-0">
        <p className="leading-none overflow-ellipsis overflow-hidden text-[14px] text-nowrap whitespace-pre">{` Rothschild College`}</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="relative shrink-0 size-[28px]" data-name="Avatar">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 28 28">
        <g clipPath="url(#clip0_5_3728)" id="Avatar">
...
function Frame15() {
  return (
    <div className="content-stretch flex flex-col gap-[16px] items-center relative shrink-0 w-full">
      <div className="font-['Geist:SemiBold',_sans-serif] font-semibold leading-[0] relative shrink-0 text-[24px] text-center text-nowrap text-white tracking-[0.12px]">
        <p className="leading-none whitespace-pre">Rideshare</p>
      </div>
      <Frame13 />
      <Frame14 />
    </div>
...
function Button({ onViewMore }: { onViewMore: () => void }) {
  return (
    <button 
      onClick={onViewMore}
      className="bg-neutral-50 relative rounded-[6px] shrink-0 w-full hover:bg-zinc-100 transition-colors" 
      data-name="Button"
    >
      <div className="flex flex-row items-center justify-center relative size-full">
        <div className="box-border content-stretch flex items-center justify-center px-[16px] py-[10px] relative w-full">
          <div className="flex flex-col font-['Geist:Medium',_sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[16px] text-black text-center text-nowrap tracking-[0.08px]">
            <p className="leading-none whitespace-pre">View More</p>
          </div>
        </div>
      </div>
    </button>
...
function Option({ onViewMore }: { onViewMore: () => void }) {
  return (
    <div className="bg-zinc-800 relative rounded-[16px] shrink-0 w-full z-[1]" data-name="Option">
      <div className="relative size-full">
        <div className="box-border content-stretch flex flex-col gap-[16px] items-start pb-[8px] pt-[12px] px-[8px] relative w-full">
          <Frame15 />
          <Button onViewMore={onViewMore} />
        </div>
      </div>
    </div>
  );
...
function Pod({ onViewMore }: { onViewMore: () => void }) {
  return (
    <div className="content-stretch flex flex-col isolate items-center relative shrink-0 w-full" data-name="Pod">
      <Group2 />
      <Option onViewMore={onViewMore} />
    </div>
  );
}
...
export default function RideScreenWithGroup({ onNavigate, showSuccessToast = false, showLeaveToast = false }: RideScreenWithGroupProps) {
  const [toastVisible, setToastVisible] = useState(showSuccessToast);

  return (
    <div className="bg-[#16161b] relative rounded-[40px] size-full" data-name="Trip 11">
      <Toast 
        message="Successfully selected group"
