...
import svgPaths from "../imports/svg-rpf08mq3uc";
        ...
        <div className="box-border content-stretch flex flex-col items-start justify-center pl-0 pr-[258.05px] py-0 relative size-full">
          <div className="flex flex-col font-['Geist:Medium',_sans-serif] font-medium justify-center leading-[0] relative shrink-0 text-[14px] text-nowrap text-white tracking-[0.07px]">
            <p className="leading-none whitespace-pre">
              AA1819<span className="text-[#00bbff]">|</span>
            </p>
          </div>
        </div>
      </div>
    ...
    <div className="absolute inset-[-7.14%]" data-name="add-circle--button-remove-cross-add-buttons-plus-circle-+-mathematics-math">
      <svg className="block size-full" fill="none" preserveAspectRatio="none" viewBox="0 0 16 16">
        <g id="add-circle--button-remove-cross-add-buttons-plus-circle-+-mathematics-math">
          <path clipRule="evenodd" d={svgPaths.p36bb600} fill="var(--fill-0, #A1A1AA)" fillRule="evenodd" id="Subtract" />
        </g>
      </svg>
    </div>
  );
      ...
      <div className="flex flex-row items-center max-w-inherit relative size-full">
        <div className="box-border content-stretch flex gap-[12px] h-[40px] items-center max-w-inherit px-[13px] py-[11.5px] relative w-full">
          <input 
            ...
            className="bg-transparent border-none outline-none flex-1 font-['Geist:Medium',_sans-serif] font-medium text-[14px] text-white tracking-[0.07px]"
            placeholder="AA1819"
          />
...
function Button({ onNext }: { onNext: () => void }) {
  return (
    <button 
      onClick={onNext}
      className="bg-neutral-50 box-border content-stretch flex h-[40px] items-center justify-center pb-[10.5px] pt-[9.5px] px-[16px] relative rounded-[6px] shrink-0 hover:bg-zinc-100 transition-colors" 
      data-name="Button"
    >
...
function Frame43({ flightNumber, setFlightNumber, onNext }: { 
  flightNumber: string; 
  ...
  return (
    <div className="content-stretch flex gap-[8px] items-center relative shrink-0 w-full">
      <Input value={flightNumber} onChange={setFlightNumber} />
...
function Frame37({ onNavigate }: { onNavigate: (screen: string) => void }) {
  const [flightNumber, setFlightNumber] = useState('AA1819');
    ...
    <div className="basis-0 grow min-h-px min-w-px relative shrink-0 w-full">
      <div className="flex flex-col items-center relative size-full">
        <div className="box-border content-stretch flex flex-col gap-[40px] items-center px-[12px] py-[40px] relative size-full">
          ...
          <Frame43 flightNumber={flightNumber} setFlightNumber={setFlightNumber} onNext={handleNext} />
        </div>
      </div>
    </div>
  );
...
export default function FlightInputScreen({ onNavigate }: FlightInputScreenProps) {
  return (
    <div className="bg-[#16161b] relative rounded-[40px] size-full" data-name="Trip 09">
      <div className="flex flex-col items-center relative size-full">
