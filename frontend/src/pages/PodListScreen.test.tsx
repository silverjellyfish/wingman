...
describe('PodListScreen', () => {
  ...
  const mockPods = [
    {
      ...
      num_members: 2,
      members: [
        { id: '1', email: 'user1@test.com', name: 'Alice Smith', createdAt: '2025-01-01' },
        { id: '2', email: 'user2@test.com', name: 'Bob Jones', createdAt: '2025-01-01' },
      ],
  ...
  describe('Initial Rendering', () => {
    it('renders the back button', () => {
      render(<PodListScreen onNavigate={mockNavigate} payload={mockPayload} />)
  ...
  describe('Pod Filtering', () => {
    it('shows no pods message when no matches found', async () => {
      ...
      await waitFor(() => {
        expect(screen.getByText(/No pods available matching your preferences/i)).toBeInTheDocument()
      })
    })
  ...
  describe('Pod Display', () => {
    ...
    it('marks first pod as recommended', async () => {
      ...
      await waitFor(() => {
        ...
        expect(screen.queryAllByText('Kirkland Hall').length).toBeGreaterThan(0)
      })
    })
  })
  ...
  describe('Group Option Cards', () => {
    it('displays member names in cards', async () => {
      ...
      await waitFor(() => {
        ...
        const hasContent = screen.queryAllByText(/Kirkland Hall/i).length > 0
  ...
  describe('Edge Cases', () => {
    ...
    it('handles missing flight data', () => {
      ...
      render(<PodListScreen onNavigate={mockNavigate} payload={payloadWithoutFlight} />)
      expect(screen.getByText('Groups')).toBeInTheDocument()
    })
    ...
    it('shows alert when pod is accepted', async () => {
      ...
      await waitFor(() => {
        ...
        const acceptButton = Array.from(acceptButtons).find(btn =>
          btn.textContent?.includes('Accept') || btn.textContent?.includes('Join')
        )
