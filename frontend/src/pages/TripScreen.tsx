      ...
      const fetchPods = async () => {
         ...
         try {
            ...
            const tripsData: Trip[] = pods.map((pod) => {
               ...
               const flightDateObj = new Date(rawFlightDate);
               const memberFlightDate = `${String(
                  flightDateObj.getMonth() + 1
               ).padStart(2, "0")}/${String(flightDateObj.getDate()).padStart(
                  2,
                  "0"
               )}/${flightDateObj.getFullYear()}`;

               const listPeopleIds: GroupMember[] = pod.members.map(
                  (m, idx) => {
                     return {
               ...
               const flight: Flight = {
                  ...
                  boarding:
                     currentUserMember?.boardingTime || pickupTimeFormatted,
                  launch:
                     currentUserMember?.departureTime || pickupTimeFormatted,
                  landing:
                     currentUserMember?.arrivalTime || pickupTimeFormatted,
            ...
            <div className="content-stretch flex flex-col gap-[40px] items-center pb-[40px] pt-[80px] px-[10px] w-full">
               ...
               <div className="w-full">
                  {loading ? (
                           ...
                           >
                              ...
                              <ExpandedPodCard
                                 flight={{
                                    ...
                                    arrivalTime: trip.flight.landing || "",
                                    destination:
                                       trip.flight.destination || "Unknown",
                                    route: trip.flight.route || "",
                                    airports: trip.flight.airports || "",
                                    pickupTimeDisplay:
                                       trip.pod.pickupTimeDisplay,
                                 }}
                                 pod={trip.pod}
                                 onLeave={() => {
                     ...
                     pastTrips.map((trip) => (
                        <div key={trip.pod.id} className="mb-[30px]">
                           <PriorTripCard
                              flight={{
                                 ...trip.flight,
                                 boardingTime: trip.flight.boarding || "",
